...
// src/availability.ts
async function checkAvailability(intent, zipCode, env) {
  const requestBody = {
    relativeDate: intent.datePreference,
    requestedTime: intent.timePreference,
    pickupZipCode: zipCode,
    currentTimeUTC: (/* @__PURE__ */ new Date()).toISOString(),
    flexibilityHours: 2,
    flexibilityDays: 7
    // Increased from 3 to 7 for broader search
  };
  console.log("[AVAILABILITY_REQUEST]", JSON.stringify(requestBody));
  try {
    const request = new Request("https://internal/availability-smart", {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      },
    ...
    const response = await env.AVAILABILITY_API.fetch(request);
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(`Availability API error (${response.status}): ${errorText}`);
    }
    const data = await response.json();
    console.log("[AVAILABILITY_RESPONSE]", JSON.stringify({
      success: data.success,
      slotsCount: data.alternatives?.length || 0,
      requestedSlotAvailable: data.requestedSlot?.available,
      requestedSlotTime: data.requestedSlot?.displayTime,
      alternativesPreview: data.alternatives?.slice(0, 3).map((s) => s.displayTime) || [],
      conversationalResponse: data.conversationalResponse,
      timezone: data.timezone,
      calculatedDate: data.calculatedDate?.confirmationText
    }));
    const slots = [];
    if (data.requestedSlot && data.requestedSlot.available) {
      slots.push(data.requestedSlot);
    }
    if (data.alternatives && Array.isArray(data.alternatives)) {
      slots.push(...data.alternatives);
    }
    return slots;
  } catch (error) {
    console.error("[AVAILABILITY_ERROR]", {
      error: error instanceof Error ? error.message : String(error),
      stack: error instanceof Error ? error.stack : void 0,
      intent: {
        date: intent.datePreference,
...
// src/orchestrator.ts
async function orchestrateScheduling(userMessage, zipCode, env, conversationHistory, customSystemPrompt) {
  const intent = await extractQuickIntent(userMessage, env, conversationHistory, customSystemPrompt);
  console.log("[INTENT]", JSON.stringify(intent));
  const allSlots = await checkAvailability(intent, zipCode, env);
  const available = allSlots.filter((slot) => slot.sameDay === true);
  const proposed = allSlots.filter((slot) => slot.sameDay !== true);
  const message = available.length > 0 ? `I have ${available.map((s) => s.displayTime).join(", ")} available. Which works best for you?` : proposed.length > 0 ? `I have ${proposed.slice(0, 3).map((s) => s.displayTime).join(", ")} available. Which works best for you?` : `I have availability coming up. Which works best for you?`;
...
__name(orchestrateScheduling, "orchestrateScheduling");
async function extractQuickIntent(userMessage, env, conversationHistory, customSystemPrompt) {
...
`;
  ...
  const defaultSystemPrompt = `Today is ${todayFormatted}.
${conversationContext}
Extract scheduling intent. Return JSON:
- datePreference: "tomorrow", "next Tuesday", "Thursday in 3 weeks", or "today" for ASAP/anytime WITHOUT a specific date
- timePreference: "morning" (6-12), "afternoon" (12-17), "evening" (17-21), "earliest", or "HH:MM"
- constraints: array of time constraints (exclude, before, after)
- confidence: 0-1

IMPORTANT:
- If ONLY "anytime", "whenever", "flexible" with NO specific date \u2192 "today" + "earliest"
- If specific date + "anytime" (e.g., "Saturday anytime") \u2192 use that date + "earliest"

Examples:
"thursday afternoon after lunch" \u2192 {"datePreference": "next Thursday", "timePreference": "afternoon", "constraints": [{"type": "after", "time": "lunch time", "reason": "after lunch time"}], "confidence": 0.95}
"as soon as possible" \u2192 {"datePreference": "today", "timePreference": "earliest", "constraints": [], "confidence": 0.99}
"anytime" \u2192 {"datePreference": "today", "timePreference": "earliest", "constraints": [], "confidence": 0.99}
"whenever works" \u2192 {"datePreference": "today", "timePreference": "earliest", "constraints": [], "confidence": 0.99}
"Saturday anytime" \u2192 {"datePreference": "this Saturday", "timePreference": "earliest", "constraints": [], "confidence": 0.95}
"tomorrow morning" \u2192 {"datePreference": "tomorrow", "timePreference": "morning", "constraints": [], "confidence": 0.99}`;
  ...
  try {
    ...
    console.error("[INTENT_EXTRACTION_ERROR]", {
      error: error instanceof Error ? error.message : String(error)
    });
    return {
...
__name(extractQuickIntent, "extractQuickIntent");
...
var src_default = {
  async fetch(request, env) {
    ...
    try {
      return await handleScheduling(request, env);
    } catch (error) {
...
async function handleScheduling(request, env) {
  ...
  if (!zipCodePattern.test(body.zipCode)) {
    return jsonResponse(
      {
        success: false,
        error: "Invalid zipCode format. Must be 5 digits (e.g., 98004) or ZIP+4 format (e.g., 98004-1234)",
        message: "I need a valid zip code to check availability. Could you provide that?",
        available: [],
        proposed: []
      },
      400
  ...
  const { intent, available, proposed, message } = await orchestrateScheduling(
    body.userMessage,
    body.zipCode,
    env,
...
__name(handleScheduling, "handleScheduling");
